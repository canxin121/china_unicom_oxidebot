...
查询和定时通知都会生成紧凑的结构化报告：账号与套餐、通用/免流的已用和剩余、区间/今日变化、必要的结转或不限量数据、最多三条套餐明细，以及新增套餐和告警。它不再输出冗长的全量分类列表。

报告以 OxideBot 的 `RichText` 表达；通过官方 Telegram adapter 发送时会安全渲染为 Telegram `MarkdownV2`：标题和分区为粗体，账号 ID 与流量数值为等宽代码。套餐名称和运营商返回文本会自动转义，避免 Markdown 特殊字符导致发送失败。
